...
if not exist "%PROJECT_DIR%\.env" (
    ...
    echo.
    echo === 텔레그램 봇 및 제미나이 설정 ===
    echo.

    set /p BOT_TOKEN="텔레그램 봇 토큰 입력 (BotFather에서 발급): "
    set /p USER_ID="허용할 사용자 ID 입력 (쉼표로 구분): "
    set /p GEMINI_KEY="구글 제미나이 API 키 입력 (aistudio.google.com): "
    ...
    echo       ✅ .env 파일 존재함
    echo       ⚠️  GEMINI_API_KEY가 설정되어 있는지 확인해주세요!
)
echo.
...
echo      register_scheduler.bat 실행
echo.
echo   2. 수동 테스트:
echo      python quick_check.py  (메시지 확인)
echo      gemini_autoexecutor.bat (에이전트 실행)
echo.
pause
