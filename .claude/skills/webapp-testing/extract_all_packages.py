...
with sync_playwright() as p:
    ...
    for num in range(1, 333):
        district_num = f"{num:03d}"
        print(f"\n{'='*60}")
        print(f"[{num}/332] {district_num}번 처리 중...")
        print(f"{'='*60}")

        # 검색 (새로고침 없이 연속 검색)
        search_input = page.locator('#sear_package_name')
        search_input.fill('')  # 먼저 지우기
        time.sleep(0.2)
    ...
    print(f"총: {success_count + no_data_count + not_found_count}/332")
